...
function  conecta_ajax() {
    ...
    /*  xmlhttp
       J? vimos que para nos comunicarmos com um documento web via AJAX 
     ...
     */
     ...
     this.connect = function(sURL, sMethod, sVars, fnDone)  {
			...
			try {
			      ...
			      /*  Nesta fun??o voc? ir? inserir uma fun??o(callback)
                      para que toda vez que o estado da requisi??o mude 
			      ...
			      xmlhttp.onreadystatechange = function() {
			            if ( xmlhttp.readyState==4 && !bComplete) {
                            ...
                            if (xmlhttp.status == 403) {
                                   alert("Diret?rio n?o encontrado") // Diret?rio n?o encontrado
                                   m_erro=1;
                            } else if (xmlhttp.status == 404) {
                                  alert("Problema de permiss?o") // Problema de permiss?o
                                  m_erro=1;
					       ...
					       if ( xmlhttp.status==200 ) {	   
						        ...
						        /*  aqui a input hidden de id=?dados? 
                                    recebe um valor que din?micamente 
                                    via c?digo Javascript: cria um 
                                    objeto de refer?ncia ?  tag input hidden   */
